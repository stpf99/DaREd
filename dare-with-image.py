...
def load_image_grid(image_path, invert=True):
    """Wczytuje obraz i konwertuje go na siatkę 64x64 binarną.
    
    Args:
        image_path (str): Ścieżka do pliku obrazu
        invert (bool): Czy odwrócić kolory (czarny na biały i odwrotnie)
        
    Returns:
        list[list[int]]: Siatka 64x64 z wartościami 0 i 1
    """
    # Wczytaj obraz i przekonwertuj na tryb czarno-biały
    img = Image.open(image_path).convert('L')
    
    # Przeskaluj do rozmiaru 64x64
    img = img.resize((64, 64))
    
    # Konwertuj na tablicę numpy
    img_array = np.array(img)
    
    # Normalizuj i konwertuj na wartości binarne (0 i 1)
    # Przy invert=True odwracamy logikę: ciemne piksele stają się 1, jasne 0
    if invert:
        binary_grid = [[1 if pixel <= 128 else 0 for pixel in row] for row in img_array]
    else:
        binary_grid = [[1 if pixel > 128 else 0 for pixel in row] for row in img_array]
...
if __name__ == "__main__":
    # Możesz wybrać czy chcesz wczytać obraz czy wygenerować losową siatkę
    use_image = True  # Zmień na False, aby używać losowej siatki
    
    if use_image:
        # Teraz domyślnie odwraca kolory (czarny staje się aktywny)
        grid = load_image_grid("image.jpg")
    else:
